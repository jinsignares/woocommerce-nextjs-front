import { CartItem, Product, Cart, Customer } from '@/types';
...
export const authorizeUser = async (req: NextApiRequest) => {
  try {
    ...
    const key: any = jwt.verify(
      session.user.key,
      process.env.WP_JWT_AUTH_SECRET_KEY!,
      {
        algorithms: ['HS256'],
...
export const initCart = async () => {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_WP_API_URL}/wp-json/cocart/v1/get-cart`,
  );
